...
<head>
    ...
    <style>
        ...
        :root {
            ...
            /* 2025 新课标学科色谱 (糖果色) */
            --c-mat: #4fc3f7; --c-eng: #ce93d8; --c-chn: #f48fb1;
            --c-phy: #9575cd; --c-che: #ff8a65; --c-bio: #a5d6a7;
            --c-pol: #e57373; --c-his: #8d6e63; --c-geo: #4db6ac;
        }

        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; user-select: none; outline: none; } body {
        ...
        /* === 2. 顶部灵动岛 === */
        .header {
            padding: 10px 20px; padding-top: calc(10px + var(--safe-top));
        ...
        @keyframes pulse { 0% {opacity:1;} 50% {opacity:0.5;} 100% {opacity:1;} }/* 搜索框 (四级联动感知) */
        .search-box {
        ...
        /* 动态着色 */
        .chip.active[data-s="数学"] { background: var(--c-mat); }
        .chip.active[data-s="英语"] { background: var(--c-eng); }
        .chip.active[data-s="语文"] { background: var(--c-chn); }
        .chip.active[data-s="物理"] { background: var(--c-phy); }
        ...
        .dock-btn:active { opacity: 1; transform: scale(1.2); color: var(--primary); }/* 猫娘悬浮球 (葵) */
        .neko-fab {
        ...
        .chat-head { padding: 15px 20px; border-bottom: 1px solid #f0f0f0; display: flex; justify-content: space-between; align-items: center; }
        ...
        .score-badge { display: inline-block; background: #ffeb3b; color: #f57f17; padding: 2px 8px; border-radius: 12px; font-weight: bold; font-size: 12px; margin-bottom: 5px; }.chat-input-area { padding: 15px; background: #fff; display: flex; gap: 10px; align-items: center; padding-bottom: calc(15px + var(--safe-bottom)); }
        ...
        .panel.show { transform: translateY(0); } /* 详情内容 */
        ...
        .ipt:focus { background: #fff; box-shadow: 0 0 0 2px var(--primary); }
        .btn-save { width: 100%; padding: 18px; background: var(--primary); color: #fff; border: none; border-radius: 20px; font-size: 17px; font-weight: 800; box-shadow: 0 8px 20px rgba(255, 128, 171, 0.4); transform: translateY(0); transition: 0.2s; margin-top: 10px; }
...
<body>
    ...
    <div id="boot">
        <div class="loader-flower">🌸</div> <div style="margin-top:20px; font-weight:900; color:#ff80ab; font-size:20px;">Aoi V_Final</div>
    ...
    <div class="header">
        <div class="top-row">
            <div class="logo">Sakura Aoi</div>
            <div class="status-pill"><span class="dot"></span><span id="dbStat">Sync...</span></div>
        </div>
        <div class="search-box">
            <span style="font-size:18px; margin-right:5px; color:var(--primary);">🔍</span>
            <input id="sInput" class="search-ipt" placeholder="支持空格: 数学 必修1 导数...">
        </div>
        <div class="filter-container">
            <div class="filter-row" id="filterSub"></div>
    ...
    <div class="viewport" id="viewport">
        ...
        <div id="pool"></div>
        <div id="empty" style="text-align:center; color:#ccc; margin-top:150px; display:none; font-weight:bold;">(｡•́︿•̀｡) 暂无笔记</div>
    </div>

    <div class="dock">
        <button class="dock-btn" onclick="io.export()">
            <span style="font-size:22px">☁️</span><span>备份</span>
        </button>
        <div class="fab" onclick="ui.openEdit()">＋</div>
        <button class="dock-btn" onclick="document.getElementById('f').click()">
            <span style="font-size:22px">📂</span><span>恢复</span>
        </button>
    </div>
    <input type="file" id="f" hidden onchange="io.import(this)"><!-- 猫娘入口 -->
    <div class="neko-fab" onclick="neko.open()">🐱<div class="neko-badge">葵</div></div>

    <!-- AI 面板 -->
    <div class="mask" id="chatMask" onclick="neko.close()"></div>
    <div class="chat-panel" id="chatPanel">
        <div class="chat-head">
            <span style="font-weight:900; color:#333;">🐱 葵 (Aoi) - 你的专属导师</span>
            <button onclick="neko.close()" style="border:none;background:none;color:#999;font-size:20px;">×</button>
        </div>
        <div class="chat-body" id="chatBody">
            <div class="msg neko">
                主人好！我是葵 (Aoi) 。<br>
                <b>✨ 我能帮你：</b><br>
                1. <b>智能判卷</b>：发送“背诵《阿房宫赋》”，然后默写，我会逐字纠错打分。<br>
                2. <b>深度讲解</b>：点击笔记的“🎧 讲解”，我会用清北逻辑为你剖析。<br>
                3. <b>全库秒搜</b>：发送“搜 数学 导数”，精准定位知识点。
            </div>
        </div>
        <div class="chat-input-area">
            <input id="chatIpt" class="chat-ipt" placeholder="和葵说点什么...">
            <button class="chat-send" onclick="neko.send()">↑</button>
        </div>
    </div>
    ...
    <div class="panel" id="detailPanel" style="height:92vh;">
        <div class="d-nav">
            <button class="d-btn" onclick="ui.closeDetail()" style="color:#999;">← 返回</button>
            <span style="font-weight:900; color:var(--text);">笔记详情</span>
            <button class="d-btn" style="color:#ff5252" onclick="core.del()">删除</button>
        </div>
        <div class="d-scroll">
            <div class="d-card">
                <span class="d-label">💡 核心考点</span>
                <div class="d-txt" id="dApp" style="font-weight:900; font-size:22px;"></div>
            </div> <div class="d-card">
                <span class="d-label">📘 详细解析 / 公式</span>
                <div class="d-txt math" id="dKwd"></div>
            </div>
            <div class="d-card" id="dErrBox">
                <span class="d-label" style="color:#ef5350">⚠️ 易错 / 导师点拨</span>
                <div class="d-txt" id="dErr" style="color:#5d4037"></div>
            </div>
            <img id="dImg" class="d-img" style="display:none">
            <button onclick="neko.speakCurrent()" style="width:100%; padding:15px; background:#f3e5f5; border:none; border-radius:16px; font-weight:bold; color:#7b1fa2; margin-top:10px; box-shadow:0 4px 10px rgba(0,0,0,0.05);">🎧 让葵讲解 (TTS)</button>
        </div>
    </div>
    ...
    <div class="panel" id="editPanel">
        <div style="text-align:center; font-weight:900; color:var(--text); margin-bottom:10px; font-size:18px;">✨ 知识录入</div>
        <div class="ipt-row">
            <select id="iSub" class="ipt" style="flex:1; font-weight:bold;" onchange="ui.upVer()"></select>
            <select id="iVer" class="ipt" style="flex:1" onchange="ui.upMod()"></select>
            <select id="iMod" class="ipt" style="flex:1"></select>
        </div>
        <input id="iApp" class="ipt" placeholder="💡 标题 (必填)">
        <textarea id="iKwd" class="ipt" style="height:120px" placeholder="📘 内容 (支持公式 AsciiMath)"></textarea>
        <textarea id="iErr" class="ipt" style="height:80px" placeholder="⚠️ 备注 / 参考图说明"></textarea>
        <div style="display:flex; justify-content:space-between; align-items:center; padding:0 10px;">
            <span style="font-size:12px; color:#999;" id="imgTip">📷 暂无图片</span>
            <buttononclick="document.getElementById('cam').click()" style="background:#fff; border:1px solid #ddd; padding:6px 15px; border-radius:12px; font-size:12px; font-weight:bold; color:var(--text);">添加图片</button>
        </div>
        <input type="file" id="cam" hidden accept="image/*" onchange="core.img(this)">
        <button class="btn-save" onclick="core.save()">保存到永恒库</button>
    </div>

<script>
    /**
     * V_Final_Aoi 终极内核
     * 包含: Worker多线程, Neko-Core 2.0 (Diff算法), 2025全教材索引, 物理粒子
     */
    
    const CFG = {
        sub: ['全部','数学','英语','语文','物理','化学','生物','政治','历史','地理'],
        col: {'数学':'#42a5f5','英语':'#ab47bc','语文':'#f06292','物理':'#9575cd','化学':'#ef5350','生物':'#81c784','政治':'#ff7043','历史':'#8d6e63','地理':'#26a69a'},
        // 2025 新课标全网教材映射
        ver: {
            '数学':['通用','人教A版','人教B版','北师大版','苏教版','湘教版'],
            '英语':['通用','人教版','外研版','北师大版','牛津版','高考3500词'],
            '语文':['通用','人教版','苏教版','鲁教版','语文版'],
            '物理':['通用','人教版','鲁科版','教科版','沪科版'],
            '化学':['通用','人教版','鲁科版','苏教版'],
            '生物':['通用','人教版','苏教版','北师大版'],
            '政治':['通用','人教版 (统编)'],
            '历史':['通用','人教版 (中外纲要)'],
            '地理':['通用','人教版','湘教版','鲁教版','中图版']
        },
        mod: ['必修1','必修2','必修3','必修4','选择性必修1','选择性必修2','选择性必修3','选修'],
        H: 118, N: 16
    };
    ...
    const workerBlob = new Blob([`
        ...
        self.onmessage = async e => {
            ...
            try {
                ...
                else if(t==='E') post(u, await all());else if(t==='M') { await batch(d); idx=await load(); post(u,1); }
        ...
        function search({s,v,m,k}){
            let r=idx;
            if(s!=='全部')r=r.filter(x=>x.s===s);
            if(v!=='全部')r=r.filter(x=>x.v===v);
            if(m!=='全部')r=r.filter(x=>x.m===m);
        ...
        async function aiQuery(q) {
            if(q.includes('搜')) {
                const k = q.replace('搜','').replace('索','').trim();
                const res = idx.filter(x=>(x.a+x.k).includes(k)).slice(0,3);
                return { type: 'SEARCH', data: res };
            }
            if(q.includes('背') || q.includes('抽')) {
                const sub = q.includes('英语')||q.includes('单词')?'英语':'语文';
                const arr = idx.filter(x=>x.s===sub);
                if(!arr.length) return { type: 'MSG', msg: '喵~ 你的'+sub+'库是空的，先去添加点笔记吧！' };
            ...
            return { type: 'MSG', msg: '喵？你可以说“背单词”、“抽查语文”或者“搜索 xxx”哦~' };
        }

        function diffCheck(ans, std) {
            let s1=ans.trim(), s2=std.trim();
            if(s1===s2) return { score:100, html:'完美！全对！' };
            let html = ''; let i=0;
            for(let char of s2) {
    ...
    const api = (t,d)=>new Promise((r,j)=>{const u=Math.random().toString(36).slice(2);const h=e=>{if(e.data.u===u){worker.removeEventListener('message',h);e.data.e?j(e.data.e):r(e.data.d)}};worker.addEventListener('message',h);worker.postMessage({t,d,u})});let S = { sub:'全部', ver:'全部', mod:'全部', ids:[], cur:null, img:null, h:0, scroll:0 };
    let quizItem = null;
            ...
            api('GET', ids).then(items => {
                items.forEach((item, i) => {
                    ...
                    if(el.dataset.idx != (start+i)) {
                        ...
                        el.dataset.id = item.id;el.querySelector('.c-tag').style.backgroundColor = CONFIG.col[item.sub]||'#999';
                        el.querySelector('.c-tag').innerText = item.sub;
                        el.querySelector('.c-ver').innerText = item.ver + ' · ' + item.mod;
    ...
    const neko = {
        ...
        send: async () => {
            ...
            if(quizItem) {
                ...
                neko.addMsg(`<span class="score-badge">${res.res.score}分</span><br>标准：${quizItem.kwd}<br>你的：${res.res.html}`, 'neko');
                if(res.res.score < 85) neko.speakText("哎呀，还要加油哦！要注意红色标记的地方。");
                else neko.speakText("太棒了，全对！");
                quizItem = null;
                return;
            }
            ...
            if(res.type === 'MSG') {
                ...
                neko.addMsg(`找到啦：\n${res.data.map(x=>"· "+x.app).join('\n')}`, 'neko');
            } else if(res.type === 'QUIZ') {
                quizItem = res.item;
                neko.addMsg(`【抽查】\n科目：${quizItem.s}\n题目：${quizItem.a}\n请背诵内容：`, 'neko');
                neko.speakText("请背诵："+quizItem.a);
            }
        },addMsg: (html, type) => {
            const d = document.createElement('div');
        ...
        speakCurrent: () => {
            if(!S.cur) return;
            const intro = `现在讲解，${S.cur.sub}，${S.cur.app}。`;
            const content = `核心内容是：${S.cur.kwd}。`;
            const tip = S.cur.err ? `请特别注意：${S.cur.err}` : '';
    ...
    const core = {
        save: async () => {
            ...
            if(!d.app) return alert('标题不能为空哦');
            await api('W', d);
            ui.closeEdit(); search();
        },
        del: async () => { if(confirm('确认删除？')) { await api('D', S.cur.id); ui.closeDetail(); search(); } },
        inject: async () => {
            const d = [
                {id:1,sub:"数学",ver:"人教A版",mod:"必修1",app:"【函数】单调性定义",kwd:"x1<x2 => f(x1)<f(x2) (增)",err:"注意定义域"},
                {id:2,sub:"英语",ver:"高考3500词",mod:"必修1",app:"abandon",kwd:"v. 抛弃，放弃",err:"abandon oneself to 沉溺于"},
                {id:3,sub:"语文",ver:"人教版",mod:"必修上",app:"【古诗】短歌行",kwd:"对酒当歌，人生几何",err:"曹操"}
            ];
            await api('M', d);
        },
        img: (ipt) => {const f = ipt.files[0]; if(!f) return;
            ...
            r.onload = e => {
                ...
                img.onload = () => {
                    ...
                    document.getElementById('imgTip').innerText = '✅ 已压缩';
                }
            }; r.readAsDataURL(f);
        }
    };
        ...
        import: (ipt) => {
            ...
            r.onload = async e => {
                try {
                    ...
                    if(confirm(`导入${d.length}条？`)) { await api('M', d); search(); alert('成功 ✨'); }
                } catch(e){ alert('文件错啦'); }
            }
        }
    };
    ...
    const ui = {
        init: () => {document.getElementById('filterSub').innerHTML = CFG.sub.map(s => 
        ...
        setSub: (s) => { S.sub = s; S.ver='全部'; S.mod='全部'; ui.renderVer(); ui.renderMod(); search(); },
        renderVer: () => {
            const box = document.getElementById('filterVer');
            if(S.sub==='全部') { box.style.display='none'; return; }
            box.style.display='flex';
            const vs = ['全部', ...(CFG.ver[S.sub]||['通用'])];
            box.innerHTML = vs.map(v => `<div class="chip ${v===S.ver?'active':''}" onclick="ui.setVer('${v}')">${v}</div>`).join('');
        },
        setVer: (v) => { S.ver = v; S.mod='全部'; ui.renderVer(); ui.renderMod(); search(); },
        renderMod: () => {
            const box = document.getElementById('filterMod');
            if(S.ver==='全部') { box.style.display='none'; return; }
            box.style.display='flex';
            const ms = ['全部', ...CFG.mod];
        ...
        detail: async (id) => {
            ...
            document.getElementById('dApp').innerText = i.app;document.getElementById('dKwd').innerText = i.kwd;
            document.getElementById('dErr').innerText = i.err || '无';
        ...
        upVer: () => {
            ...
            v.innerHTML = '';
            (CFG.ver[s]||['通用']).forEach(x => v.add(new Option(x,x)));
            ui.upMod();
        },
        upMod: () => {
